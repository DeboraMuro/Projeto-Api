...
INSERT INTO tb_tipos_pagamento (id, nome, descricao, ativo, desconto, prazo_processamento, informacoes_adicionais)
VALUES
...
SELECT * FROM tb_tipos_pagamento;

-- Para selecionar apenas os tipos de pagamento ativos:
SELECT * FROM tb_tipos_pagamento WHERE ativo = true;

-- Para selecionar apenas os tipos de pagamento que oferecem desconto:
SELECT * FROM tb_tipos_pagamento WHERE desconto > 0;

-- Para atualizar as informações de um tipo de pagamento existente, por exemplo, alterando sua descrição:
UPDATE tb_tipos_pagamento SET descricao = 'Pagamento por boleto bancário (vencimento em 5 dias)' WHERE id = 2;

-- Para excluir um tipo de pagamento existente:
DELETE FROM tb_tipos_pagamento WHERE id = 3;
